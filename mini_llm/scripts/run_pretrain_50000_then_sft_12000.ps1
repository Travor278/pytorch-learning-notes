...
$SftOutDir = Join-Path $RepoRoot "mini_llm\checkpoints\minimind_mini_8k_sft50k"
$PretrainLogPath = Join-Path $PretrainOutDir "train_to_50000.log"
$SftLogPath = Join-Path $SftOutDir "sft_to_12000.log"
...
Write-Host "stage 1/2: mini_llm pretrain resume -> total step 50000"
...
Write-Host "stage 2/2: mini_llm full SFT from 50000-step pretrain -> total step 12000"
...
& $Python ".\mini_llm\trainer\train_full_sft.py" `
  ...
  --tokenizer-path ".\mini_llm\tokenizer.json" `
  --out-dir ".\mini_llm\checkpoints\minimind_mini_8k_sft50k" `
